<template>
  <div class="detail">
    ...
    这个用来判断这三个字段，我们用数组存储的，通过this.serviceIds = res.serviceIds.split(",");转换为数组
  ...
  methods: {
    ...
    handlerSkuid(item, index) {
      ...
      if (this.clicked == index) {
        ...
        this.skuName=this.product.name+ item.sp1+item.sp2+"<br/>当前库存："+(item.stock-item.lockStock)+this.product.unit;
        this.skuPrice=item.price;
      //  this.product.price = this.skuPrice;
      }
    },
  },
};
